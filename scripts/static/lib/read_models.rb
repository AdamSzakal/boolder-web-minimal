module Static
  class ReadModels
    def initialize(catalog)
      @catalog = catalog
      build_indexes!
    end

    def area_page(slug)
      area = @areas_by_slug[slug]
      raise "Area not found: #{slug}" unless area

      area_id = area.fetch("id")

      {
        "area" => area,
        "popular_problems" => popular_problems_for(area_id),
        "circuits" => circuits_for(area_id),
        "poi_routes" => poi_routes_for(area_id)
      }
    end

    def problem_page(problem_id)
      problem = @problems_by_id[problem_id]
      raise "Problem not found: #{problem_id}" unless problem

      area = @areas_by_id[problem.fetch("area_id")]
      line = @lines_by_problem_id[problem_id]
      topo = line && @topos_by_id[line.fetch("topo_id")]
      circuit = problem["circuit_id"] && @circuits_by_id[problem["circuit_id"]]

      {
        "problem" => problem,
        "area" => area,
        "line" => line,
        "topo" => topo,
        "variants" => variants_for(problem),
        "circuit" => circuit,
        "circuit_previous" => circuit_neighbor(problem, -1),
        "circuit_next" => circuit_neighbor(problem, 1)
      }
    end

    def all_areas
      @catalog.areas
        .select { |a| a["published"] == true }
        .sort_by { |a| a.fetch("name") }
    end

    private

    def build_indexes!
      @areas_by_id = @catalog.areas.each_with_object({}) { |a, h| h[a["id"]] = a }
      @areas_by_slug = @catalog.areas.each_with_object({}) { |a, h| h[a["slug"]] = a }
      @problems_by_id = @catalog.problems.each_with_object({}) { |p, h| h[p["id"]] = p }
      @problems_by_area = @catalog.problems.group_by { |p| p["area_id"] }
      @problems_by_parent = @catalog.problems.select { |p| p["parent_id"] }.group_by { |p| p["parent_id"] }
      @circuits_by_id = @catalog.circuits.each_with_object({}) { |c, h| h[c["id"]] = c }
      @topos_by_id = @catalog.topos.each_with_object({}) { |t, h| h[t["id"]] = t }
      @lines_by_problem_id = @catalog.lines.each_with_object({}) { |l, h| h[l["problem_id"]] ||= l }
      @pois_by_id = @catalog.pois.each_with_object({}) { |p, h| h[p["id"]] = p }
      @poi_routes_by_area = @catalog.poi_routes.group_by { |r| r["area_id"] }

      # Index for circuit navigation: [circuit_id, number_string] -> problem (main only)
      @circuit_problems = {}
      @catalog.problems.each do |p|
        next unless p["circuit_id"] && p["circuit_number"]
        next if p["circuit_letter"] && !p["circuit_letter"].empty?
        @circuit_problems[[p["circuit_id"], p["circuit_number"]]] = p
      end
    end

    def popular_problems_for(area_id)
      (@problems_by_area[area_id] || [])
        .select { |p| p["featured"] == true && p["location"] }
        .sort_by { |p| [-(grade_sort_key(p["grade"])), -(p["popularity"] || 0)] }
    end

    def circuits_for(area_id)
      circuit_ids = (@problems_by_area[area_id] || [])
        .filter_map { |p| p["circuit_id"] }
        .uniq

      circuit_ids.filter_map { |id| @circuits_by_id[id] }
    end

    def poi_routes_for(area_id)
      (@poi_routes_by_area[area_id] || []).map do |route|
        route.merge("poi" => @pois_by_id[route.fetch("poi_id")])
      end
    end

    def variants_for(problem)
      if problem["parent_id"]
        parent = @problems_by_id[problem["parent_id"]]
        children = @problems_by_parent[parent&.fetch("id")] || []
        ([parent] + children).compact - [problem]
      else
        @problems_by_parent[problem.fetch("id")] || []
      end
    end

    def circuit_neighbor(problem, direction)
      return nil unless problem["circuit_id"] && problem["circuit_number"]

      target_number = (problem["circuit_number"].to_i + direction).to_s
      @circuit_problems[[problem["circuit_id"], target_number]]
    end

    GRADE_VALUES = %w[
      1a 1a+ 1b 1b+ 1c 1c+
      2a 2a+ 2b 2b+ 2c 2c+
      3a 3a+ 3b 3b+ 3c 3c+
      4a 4a+ 4b 4b+ 4c 4c+
      5a 5a+ 5b 5b+ 5c 5c+
      6a 6a+ 6b 6b+ 6c 6c+
      7a 7a+ 7b 7b+ 7c 7c+
      8a 8a+ 8b 8b+ 8c 8c+
      9a 9a+ 9b 9b+ 9c 9c+
    ].freeze

    def grade_sort_key(grade)
      return -1 unless grade
      GRADE_VALUES.index(grade) || -1
    end
  end
end
